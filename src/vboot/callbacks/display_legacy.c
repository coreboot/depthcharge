...
_Static_assert(!CONFIG(MENU_UI), "MENU_UI not allowed");
_Static_assert(CONFIG(LEGACY_MENU_UI) + CONFIG(LEGACY_CLAMSHELL_UI) == 1,
	       "Exactly one of LEGACY_MENU_UI or LEGACY_CLAMSHELL_UI must be"
	       " set");
...
{
	int ret = vboot_draw_legacy_screen(screen_type, locale, data);
	if (ret != VB2_SUCCESS)
...
{
	int ret = vboot_draw_legacy_menu(screen_type, locale,
