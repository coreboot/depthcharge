...
#include "board/setzer/device_nvs.h"
...
static int board_setup(void)
{
	...
	if (nvs->scc_en[SCC_NVS_MMC])
		emmc = new_mem_sdhci_host((void *)nvs->scc_bar0[SCC_NVS_MMC],
			SDHCI_PLATFORM_NO_EMMC_HS200, emmc_sd_clock_min, emmc_clock_max);
	else
		emmc = new_pci_sdhci_host(PCI_DEV(0, 0x10, 0), SDHCI_PLATFORM_NO_EMMC_HS200,
