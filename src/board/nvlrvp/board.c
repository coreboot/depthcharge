...
#include "board/nvlrvp/include/variant.h"
...
{
	return novalake_get_gpe(GPE0_DW2_16); /* GPP_F16 */
}
...
{
	static const struct audio_config config;
...
static void flash_setup(void)
{
	if (CONFIG(DRIVER_FLASH_FAST_SPI)) {
		/* 32MB SPI Flash */
		uintptr_t mmio_base = pci_read_config32(PCI_DEV_SPI, PCI_BASE_ADDRESS_0);
