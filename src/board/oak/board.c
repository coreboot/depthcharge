...
static int board_setup(void)
{
	...
	MtkMmcHost *emmc = new_mtk_mmc_host(0x11230000, 0, 200 * MHz, 50 * MHz,
					    emmc_tune_reg, 8, 0, NULL,
					    MTK_MMC_V1);
	GpioOps *card_detect_ops = new_gpio_not(new_mtk_gpio_input(PAD_EINT1));
	MtkMmcHost *sd_card = new_mtk_mmc_host(0x11240000, 0, 200 * MHz, 25 * MHz,
	...
	if (display_init_required())
		display_set_ops(new_mtk_display(
				oak_backlight_update, 0x1400c000, 1, 0));
