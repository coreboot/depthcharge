...
#define GSC_SOC_INT_ODL	84
#define EC_SOC_INT_ODL	90

__weak const struct storage_config *variant_get_storage_configs(size_t *count)
{
	static const struct storage_config storage_configs[] = {
		/* Coreboot devicetree.cb: gpp_bridge_2_3 = 02.3 */
		{ .media = STORAGE_SDHCI, .pci_dev = PCI_DEV(0, 0x02, 0x03)},
...
/* Audio Configuration */
#define AUDIO_I2C_MMIO_ADDR	AP_I2C3_ADDR
...
static int board_setup(void)
{
	...
	/* Set up Dauntless on I2C2 */
	DesignwareI2c *i2c_gsc = new_designware_i2c(
		AP_I2C2_ADDR, 400000, AP_I2C_CLK_MHZ);
