...
#include "board/veyron_jerry/power_ops.h"
...
static int board_setup(void)
{
	...
	RkGpio *lid_switch = new_rk_gpio_input(GPIO(0, A, 6));
	RkGpio *ec_in_rw = new_rk_gpio_input(GPIO(0, A, 7));
	RkGpio *pwr_key_l = new_rk_gpio_input(GPIO(0, A, 5));
	flag_replace(FLAG_LIDSW, &lid_switch->ops);
	flag_install(FLAG_ECINRW, &ec_in_rw->ops);
	flag_replace(FLAG_PWRSW, new_gpio_not(&pwr_key_l->ops));
