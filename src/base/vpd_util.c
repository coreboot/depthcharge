 ...
 */
int decodeVpdString(const int32_t max_len, const uint8_t *input_buf,
                    int32_t *consumed, VpdDecodeCallback callback,
                    void *callback_arg)
{
  int type;
  int32_t key_len, value_len;
                             ...
                             void *arg)
{
  ...
  if (key_len != result->key_len || memcmp(key, result->key, key_len) != 0)
...
const void *vpd_find(const char *key, int *size)
{
  ...
  if (!vpd || !vpd->ro_size) return NULL;
