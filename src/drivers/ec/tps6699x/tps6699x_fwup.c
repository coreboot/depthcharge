...
#define TPS_DATA_AT(block) (TPS_DATA_METADATA_OFFSET_AT(block) + TPS6699X_METADATA_LENGTH)
 ...
 */
static ssize_t read_file_offset(Tps6699x *me, int offset, const uint8_t **buf, int len)
{
	/* Exceed size of file. */
...
static int get_appconfig_offsets(Tps6699x *me, uint16_t num_data_blocks, int *metadata_offset,
				 int *data_block_offset)
{
	int bytes_read;
	uint32_t *fw_size;

	bytes_read = read_file_offset(me, TPS6699X_FW_SIZE_OFFSET, (const uint8_t **)&fw_size,
				      sizeof(fw_size));
 ...
 * @return int 0 on success
 */
static int run_task_sync(Tps6699x *me, enum tps6699x_4cc_tasks task, union reg_data *cmd_data,
			 uint8_t *user_buf)
{
	...
	if (user_buf != NULL)
		memcpy(user_buf, cmd_data_check.data, sizeof(cmd_data_check.data));
...
static int tfud_block(Tps6699x *me, uint8_t *fbuf, int metadata_offset, int data_block_offset)
{
	...
	bytes_read = read_file_offset(me, metadata_offset, (const uint8_t **)&tfud,
	...
	if (bytes_read < 0 || bytes_read != TPS6699X_METADATA_LENGTH) {
		printf("%s: Failed to read block metadata. Wanted %d, got %d\n", me->chip_name,
		       TPS6699X_METADATA_LENGTH, bytes_read);
		return -1;
	}
	...
	if (tfud->data_block_size > TPS6699X_DATA_BLOCK_SIZE) {
		printf("%s: TFUd block size too big: 0x%x (max is 0x%x)\n", me->chip_name,
		       tfud->data_block_size, TPS6699X_DATA_BLOCK_SIZE);
		return -1;
	}
	...
	bytes_read = read_file_offset(me, data_block_offset, (const uint8_t **)&fbuf,
	...
	ret = tps6699x_stream_data(me, tfud->broadcast_address, fbuf, tfud->data_block_size);
	if (ret) {
...
int tps6699x_perform_fw_update(Tps6699x *me)
{
	...
	bytes_read = read_file_offset(me, TPS6699X_METADATA_OFFSET, (const uint8_t **)&tfui,
				      TPS6699X_METADATA_LENGTH);
	if (bytes_read < 0) {
	...
	bytes_read = read_file_offset(me, TPS6699X_HEADER_BLOCK_OFFSET, (const uint8_t **)&fbuf,
				      TPS6699X_HEADER_BLOCK_LENGTH);
	if (bytes_read < 0 || bytes_read != TPS6699X_HEADER_BLOCK_LENGTH) {
		printf("%s: Failed to read header stream. Wanted %d but got %d\n",
	...
	printf("%s: Flashing appconfig to block %d", me->chip_name, tfui->num_blocks);
	...
	ret = tfud_block(me, fbuf, appconfig_metadata_offset, appconfig_data_offset);
	if (ret) {
