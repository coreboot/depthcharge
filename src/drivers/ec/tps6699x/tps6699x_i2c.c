 ...
 */
static int tps6699x_xfer_reg(CrosECTunnelI2c *bus, uint8_t addr, uint8_t reg,
			     uint8_t *buf, uint8_t len, bool read)
{
	if (bus == NULL || buf == NULL || len < 2) {
		debug("Invalid params to %s: bus=%p, buf=%p, len=%u", __func__,
		      bus, buf, len);
		return -1;
	}
	...
	/* Number of bytes in message (excluding the register and this size
	 * field) */
	buf[1] = len - 2;
...
{
	return tps6699x_xfer_reg(me->bus, me->chip_info.i2c_addr, TPSREG_CMD1,
...
{
	return tps6699x_xfer_reg(me->bus, me->chip_info.i2c_addr, TPSREG_DATA1,
				 data->raw_value, sizeof(union reg_data),
				 /*read=*/false);
}
...
{
	return tps6699x_xfer_reg(me->bus, me->chip_info.i2c_addr, TPSREG_MODE,
				 mode->raw_value, sizeof(union reg_mode),
				 /*read=*/true);
}

int tps6699x_version_read(Tps6699x *me, union reg_version *ver)
{
	return tps6699x_xfer_reg(me->bus, me->chip_info.i2c_addr,
				 TPSREG_VERSION, ver->raw_value,
				 sizeof(union reg_version), /*read=*/true);
}
			 ...
			 const uint8_t *buf, size_t buf_len)
{
	...
	for (int chunk_offset = 0; chunk_offset < buf_len;
		...
		rv = i2c_write_raw(&me->bus->ops, broadcast_address,
				   (uint8_t *)write_start_pos, write_length);
		if (rv) {
