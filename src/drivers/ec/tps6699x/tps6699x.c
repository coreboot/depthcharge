...
static void tps6699x_set_i2c_speed(Tps6699x *me)
{
	...
	status = cros_ec_i2c_set_speed(me->bus->remote_bus,
				       CONFIG_DRIVER_EC_TPS6699X_I2C_SPEED_KHZ,
				       &old_speed_khz);
	if (status < 0) {
...
static void tps6699x_restore_i2c_speed(Tps6699x *me)
{
	...
	if (me->saved_i2c_speed_khz == 0 ||
	    CONFIG_DRIVER_EC_TPS6699X_I2C_SPEED_KHZ == 0)
		return;
...
static vb2_error_t tps6699x_ec_tunnel_status(const VbootAuxfwOps *vbaux,
					     int *protected)
{
...
/**
 * @brief Confirm the chip is present and collect FW version info via host
 * command
 *
...
static bool force_update(Tps6699x *me)
{
	...
	if (me->has_updated) {
		printf("%s: Not forcing update because an update already "
		       "happened.\n",
		       me->chip_name);
		return false;
	}
...
static vb2_error_t tps6699x_check_hash(const VbootAuxfwOps *vbaux,
				       ...
				       enum vb2_auxfw_update_severity *severity)
{
	...
	case -EC_RES_BUSY:
		...
		printf("Skipping update: Battery SoC or power state "
		       "inadequate: %d\n",
		       ret);
		*severity = VB2_AUXFW_NO_UPDATE;
		return VB2_SUCCESS;
	...
	printf("%s: Update FW from %u.%u.%u to %u.%u.%u\n", me->chip_name,
	       ...
	       PDC_FWVER_PATCH(ver_current), PDC_FWVER_MAJOR(ver_new),
	       PDC_FWVER_MINOR(ver_new), PDC_FWVER_PATCH(ver_new));
...
static vb2_error_t tps6699x_update_image(const VbootAuxfwOps *vbaux,
					 const uint8_t *image,
					 size_t image_size)
{
	Tps6699x *me = container_of(vbaux, Tps6699x, fw_ops);
 ...
 */
__weak void board_tps6699x_get_image_paths(const char **image_path,
...
Tps6699x *new_tps6699x(int ec_pd_id)
{
	...
	board_tps6699x_get_image_paths(&fw_ops.fw_image_name,
				       &fw_ops.fw_hash_name);
	...
	me->fw_ops = fw_ops;
	snprintf(me->chip_name, sizeof(me->chip_name), "tps6699x.%d",
		 me->ec_pd_id);

	return me;
}

static const VbootAuxfwOps *
new_tps6699x_from_chip_info(struct ec_response_pd_chip_info *r,
			    uint8_t ec_pd_id)
{
	Tps6699x *tps6699x;
	...
	if (tps6699x == NULL) {
		printf("Error instantiating tps6699x driver. Skipping FW "
		       "update.\n");
		return NULL;
	}
...
static int tps6699x_register(void)
{
	for (int i = 0; i < ARRAY_SIZE(aux_fw_tps6699x_info); i++)
		list_insert_after(&aux_fw_tps6699x_info[i].list_node,
				  &ec_aux_fw_chip_list);
