...
Rts545x *new_rts5453(CrosECTunnelI2c *bus, int ec_pd_id, struct ec_response_pd_chip_info_v2 *r)
{
	...
	printf("rts54xx.%d: Using PDC image '%s' with hash file '%s'\n", ec_pd_id,
	...
	me->fw_ops = fw_ops;
	snprintf(me->chip_name, sizeof(me->chip_name), "rts54xx.%d", ec_pd_id);
						...
						uint8_t ec_pd_id)
{
	...
	if (r->fw_update_flags & USB_PD_CHIP_INFO_FWUP_FLAG_NO_UPDATE) {
		printf("rts54xx.%d: Ignoring port to avoid double-upgrade\n", ec_pd_id);
		return NULL;
	}
	...
	printf("rts54xx.%d: VID/PID %04x:%04x\n", ec_pd_id, rts545x->chip_info.vid,
