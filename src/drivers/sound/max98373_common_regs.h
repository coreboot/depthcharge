// SPDX-License-Identifier: GPL-2.0
/*
 * Copyright (c) 2020, Intel Corporation.
 * Copyright 2020 Google LLC.
 */

#ifndef __DRIVERS_SOUND_MAX98373_COMMON_REGS_H__
#define __DRIVERS_SOUND_MAX98373_COMMON_REGS_H__

/* Max98373 General Control Registers */
enum {
	REG_SOFT_RESET    = 0x2000,
	REG_PCM_INTERFACE = 0x2024,
	REG_MODE_CONFIG   = 0x2025,
	REG_SAMPLE_RATE_2 = 0x2028,
	REG_DATA_INPUT_EN = 0X202B,
	REG_AMP_VOLUME    = 0x203d,
	REG_AMP_GAIN      = 0x203e,
	REG_TONE_CONFIG   = 0x2040,
	REG_SPEAKER_PATH  = 0x2043,
	REG_CLOCK_MONITOR = 0x20fe,
	REG_GLOBAL_ENABLE = 0x20ff,
};

/* REG_SPEAKER_GAIN */
enum {
	SPEAKER_GAIN_MASK = 0xf0,
	SPEAKER_GAIN_0DB  = 0x00,
	SPEAKER_GAIN_1DB  = 0x20,
	SPEAKER_GAIN_2DB  = 0x40,
	SPEAKER_GAIN_3DB  = 0x60,
	SPEAKER_GAIN_4DB  = 0x80,
	SPEAKER_GAIN_5DB  = 0x90,
	SPEAKER_GAIN_6DB  = 0xa0,
};

/* Audio Interface Mode */
enum {
	PCM_ENDPOINT_MODE  = 0,
	SNDW_ENDPOINT_MODE = 3,
};

enum {
	SPEAKER_GAIN_MAX_MASK = 0x0f,
	SPEAKER_GAIN_MAX_5P4V = 0x00,
};

/* Speaker Path Digital Volume Control */
enum {
	SPEAKER_AMP_VOLUME_MASK  = 0x7f,
	SPEAKER_AMP_VOLUME_0DB   = 0x00,
	SPEAKER_AMP_VOLUME_M1DB  = 0x02,
	SPEAKER_AMP_VOLUME_M6DB  = 0x0c,
	SPEAKER_AMP_VOLUME_M32DB = 0x40,
	SPEAKER_AMP_VOLUME_M63DB = 0x7e,
	SPEAKER_AMP_VOLUME_MUTE  = 0x7f,
};

// REG_PCM_SAMPLE_RATE
enum {
	SAMPLE_RATE_MASK    = 0x0f,
	SAMPLE_RATE_16000HZ = 0x03,
	SAMPLE_RATE_22050HZ = 0x04,
	SAMPLE_RATE_24000HZ = 0x05,
	SAMPLE_RATE_32000HZ = 0x06,
	SAMPLE_RATE_44100HZ = 0x07,
	SAMPLE_RATE_48000HZ = 0x08,
};

/* REG_SPEAKER_PATH */
enum {
	SPEAKER_ENABLE = 0x01,
};

/* REG_CLOCK_MONITOR */
enum {
	CMON_EN = 0x01,
};

/* REG_TONE_CONFIG */
enum {
	TONE_CONFIG_MASK     = 0x0f,
	TONE_CONFIG_DISABLED = 0x00,
	TONE_CONFIG_FS_4     = 0x01,
	TONE_CONFIG_FS_6     = 0x02,
	TONE_CONFIG_FS_8     = 0x03,
	TONE_CONFIG_FS_16    = 0x04,
	TONE_CONFIG_FS_32    = 0x05,
	TONE_CONFIG_FS_64    = 0x06,
	TONE_CONFIG_FS_128   = 0x07,
};

/* REG_GLOBAL_ENABLE */
enum {
	GLOBAL_ENABLE  = 0x01,
	GLOBAL_DISABLE = 0x00,
};

/* REG_SOFT_RESET */
enum {
	SOFT_RESET_TRIGGER = 0x01,
};

/* REG_PCM_INTERFACE */
enum {
	PCM_INTERFACE = 0x01,
};

/* REG_DATA_INPUT_EN */
enum {
	DATA_INPUT_EN = 0x01,
};
#endif /* __DRIVERS_SOUND_MAX98373_COMMON_REGS_H__ */
