...
#ifndef DRIVERS_SOC_COMMON_MC146818RTC_H_
#define DRIVERS_SOC_COMMON_MC146818RTC_H_

#include <libpayload.h>

#define RTC_BASE_PORT_BANK0	(CONFIG_DRIVER_CMOS_BASE_PORT_BANK_0)
#define RTC_BASE_PORT_BANK1	(CONFIG_DRIVER_CMOS_BASE_PORT_BANK_1)
...
static inline uint8_t cmos_disable_rtc(void)
{
	uint8_t control_state = cmos_read(RTC_CONTROL);
	if (!(control_state & RTC_SET))
		cmos_write_inner(control_state | RTC_SET, RTC_CONTROL);
	return control_state;
}

static inline void cmos_restore_rtc(uint8_t control_state)
{
	if (!(control_state & RTC_SET))
...
static inline uint32_t cmos_read32(uint8_t offset)
{
	uint32_t value = 0;
...
static inline void cmos_write32(uint32_t value, uint8_t offset)
{
	for (uint8_t i = 0; i < sizeof(value); ++i)
		cmos_write((value >> (i << 3)) & 0xff, offset + i);
}

#endif /*  DRIVERS_SOC_COMMON_MC146818RTC_H_ */
