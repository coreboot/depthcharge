...
#include "mt8183_ddp.h"

enum {
	DISPSYS_OVL_BASE     = 0x14008000,
	DISPSYS_OVL0_2L_BASE = 0x14009000,
	...
	DISP_REG_OVL0_2L_EN  = (DISPSYS_OVL0_2L_BASE + 0x000C),
};

static int mt8183_display_init(DisplayOps *me)
{
	...
	writel(0x01, (void *)(uintptr_t)DISP_REG_OVL0_2L_EN);

	return 0;
}

static int mt8183_display_stop(DisplayOps *me)
{
	/* Disable overlayer 0 */
	writel(0x00, (void *)(uintptr_t)DISP_REG_OVL_EN);
	writel(0x00, (void *)(uintptr_t)DISP_REG_OVL0_2L_EN);
			       ...
			       (DisplayOps *me, uint8_t enable))
{
	...
	display_ops->init = mt8183_display_init;
	display_ops->stop = mt8183_display_stop;
	if (backlight_update)
