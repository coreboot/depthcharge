...
static int __cannonlake_gpio_get(int gpio_num)
{
	uint32_t *dw_regs;
	uint32_t reg;
...
static void __cannonlake_gpio_set(int gpio_num, int value)
{
	uint32_t *dw_regs;
	uint32_t reg;
...
static int cannonlake_gpio_configure(struct GpioCfg *gpio,
...
static int cannonlake_gpio_get(struct GpioOps *me)
{
	GpioCfg *gpio = container_of(me, GpioCfg, ops);
	return __cannonlake_gpio_get(gpio->gpio_num);
}

static int cannonlake_gpio_set(struct GpioOps *me, unsigned int value)
{
	GpioCfg *gpio = container_of(me, GpioCfg, ops);
	__cannonlake_gpio_set(gpio->gpio_num, value);
	return 0;
}

static int cannonlake_gpio_get_fast(struct GpioOps *me)
{
	GpioCfg *gpio = container_of(me, GpioCfg, ops);
	return (gpio->current_dw0 >> GPIORXSTATE_SHIFT) & GPIORXSTATE_MASK;
}

static int cannonlake_gpio_set_fast(struct GpioOps *me, unsigned int value)
{
	GpioCfg *gpio = container_of(me, GpioCfg, ops);
...
GpioCfg *new_cannonlake_gpio(int gpio_num)
{
	...
	gpio->ops.get   = &cannonlake_gpio_get;
	gpio->ops.set   = &cannonlake_gpio_set;
	gpio->configure = &cannonlake_gpio_configure;
...
static int cannonlake_gpio_cleanup(CleanupFunc *cleanup, CleanupType type)
{
...
static void register_cannonlake_gpio_cleanup(GpioCfg *gpio)
{
	...
	gpio->cleanup.cleanup = &cannonlake_gpio_cleanup;
...
{
	GpioCfg *gpio = new_cannonlake_gpio(gpio_num);

	/* Restore this GPIO to original state on exit */
	register_cannonlake_gpio_cleanup(gpio);
...
GpioCfg *new_cannonlake_gpio_output(int gpio_num, unsigned int value)
{
	...
	gpio->ops.get = &cannonlake_gpio_get_fast;
	gpio->ops.set = &cannonlake_gpio_set_fast;
...
{
	return &(new_cannonlake_gpio(port)->ops);
}
