...
static void test_broken_ui_screen(void **state)
{
	...
	EXPECT_UI_DISPLAY_SUB_MENU(UI_SCREEN_RECOVERY_BROKEN, 0, 0);
	/* #1: Advanced options (open dropdown) */
	...
	EXPECT_UI_DISPLAY(UI_SCREEN_RECOVERY_BROKEN, MOCK_IGNORE, 1);
	/* #1 -> #1: Debug info */
	EXPECT_UI_DISPLAY_SUB_MENU(UI_SCREEN_RECOVERY_BROKEN, 1, 1);
	/* End of menu */
	WILL_PRESS_KEY(UI_KEY_ESC, 0);
	EXPECT_UI_DISPLAY(UI_SCREEN_RECOVERY_BROKEN, MOCK_IGNORE, 1);
	WILL_PRESS_KEY(0, 0);
...
static void test_firmware_log(void **state)
{
	...
	WILL_PRESS_KEY(UI_KEY_DOWN, 0);
	EXPECT_UI_DISPLAY_SUB_MENU(UI_SCREEN_RECOVERY_BROKEN, 1, 3);
