...
int conf_write(const char *name)
{
	...
	if (!env || !*env) {
		fd = custom_mkstemp(newname, tmpname, sizeof(tmpname));
		if (fd == -1)
			return 1;
		out = fdopen(fd, "w");
	} else {
...
int conf_write_autoconf(void)
{
	...
	i = custom_mkstemp(config_name, tmpconfig_name, sizeof(tmpconfig_name));
	if (i == -1)
	...
	i = custom_mkstemp(config_triname, tmpconfig_triname,
			   sizeof(tmpconfig_triname));
	if (i == -1)
	...
	i = custom_mkstemp(config_h, tmpconfig_h, sizeof(tmpconfig_h));
	if (i == -1)
